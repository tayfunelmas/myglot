...
import { setStatus, debounce, escapeHtml } from './util.js';
...
export async function initHome() {
    ...
    document.getElementById('home-search').addEventListener('input', debounce(() => loadItems()));
...
export async function loadCategories() {
    ...
    const selectors = [
        'category-select', 'home-category-filter',
        'practice-category-filter', 'edit-category-select',
    ];
...
async function addItem() {
    ...
    if (newCategoryInput.value.trim()) {
        ...
        payload.category_id = parseInt(categorySelect.value);
    }
...
function renderHomeItem(item) {
    const catBadge = item.category
        ...
        : '';
    const staleBadge = item.audio_stale
        ? '<span class="stale-badge">audio outdated</span>'
        : '';
    return `
...
function attachHomeListeners() {
    ...
    document.querySelectorAll('#items-list .btn-regen').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            btn.disabled = true;
            try {
                await api.regenerateAudio(parseInt(btn.dataset.id));
                await loadItems();
    ...
    document.querySelectorAll('#items-list .btn-delete').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            try {
                await api.deleteItem(parseInt(btn.dataset.id));
                await loadCategories();
                await loadItems();
            } catch (e) { alert(e.message); }
        });
...
async function saveEdit() {
    ...
    const categoryId = parseInt(document.getElementById('edit-category-select').value);
